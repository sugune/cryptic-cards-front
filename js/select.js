...
class UI {
  ...
  createRandomizedChoices = (cards) => {
    const cardElements = cards.map((card) => {
      ...
      const lengthOfChoices =
        lengthOfCorrectAnswer > 2 ? lengthOfCorrectAnswer * 2 : 4;
      ...
      card.querySelector(".select-list-definition").innerHTML =
        cardDefinitions.join(" ");
  ...
  getRandomDefinition(correctDefinitions) {
    ...
    return existingDefinition.includes(randomDefinition)
      ? this.getRandomDefinition(correctDefinitions)
      : randomDefinition;
  }
  ...
  createChoicesFunctionality = (cards) => {
    ...
    cards.forEach((card) => {
      ...
      const listDefinitionsContainer = card.querySelector(
      ...
      listDefinitionsContainer.addEventListener("click", (e) => {
        if (
          e.target.matches(".single-definition") &&
